  ...
  return (
    <div className="p-5 sm:p-6 rounded-2xl bg-white border border-[#EAE3D7] shadow-xs hover:shadow-md transition-all duration-200 flex flex-col justify-between">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs sm:text-sm font-extrabold text-slate-700 uppercase tracking-wider">{title}</p>
      ...
      {(change || description) && (
        <div className="mt-4 pt-3.5 border-t border-slate-100 flex items-center justify-between text-xs sm:text-sm text-slate-700">
          {change && (
            <span
          ...
          {description && <span className="truncate text-slate-600 text-xs font-medium">{description}</span>}
        </div>
      )}
    </div>
  );
